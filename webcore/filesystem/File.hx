...
class File {
	...
	**/
	public static function readBundleFile(
		...
		path: String,
		?onComplete: (webcore.typedarray.ArrayBuffer) -> Void,
		?onError: (String) -> Void,
		...
		if (path == null) {
			...
			var bundle = if (bundleIdentifier != null) {
				webcore.filesystem.native.CFBundle.getBundleWithIdentifier(webcore.filesystem.native.CFBundle.CFStringRef.create(bundleIdentifier));
			} else {
				webcore.filesystem.native.CFBundle.getMainBundle();
			}
			...
			var bundleResourceDirectory: String = webcore.filesystem.native.CFBundle.getResourceDirectory(bundle);
