...
import webcore.typedarray.ArrayBuffer;
import webcore.audio.native.AudioDecoder;
import webcore.audio.native.NativeAudioNode.NativeAudioNodeList;
import webcore.audio.native.MiniAudio;
import webcore.audio.native.LockedValue;

@:include('./native.h')
@:sourceFile(#if winrt './native.c' #else './native.m' #end)
@:allow(webcore.audio.AudioNode)
@:allow(webcore.audio.native.AudioDecoder)
class AudioContext {
...
class DeviceUserData {
    ...
    public final schedulingCurrentFrameBlock: webcore.audio.native.LockedValue<Int64>;
